...
//    private MyReceiver receiver;
    ...
    private void setPlayAndPauseStatus() {
        try {
            if (service.isPlaying()) {
                //暂停
                //按钮设置播放状态
...
//    }
    ...
    private void showLyric() {
        ...
        try {
            ...
            File file = new File(audioPath.substring(0,audioPath.indexOf("."))+".txt");//mnt/sdcard/audio/beijingbeijing.txt
            if(!file.exists()){
                file = new File(audioPath.substring(0,audioPath.indexOf("."))+".lrc");//mnt/sdcard/audio/beijingbeijing.lrc
            }

            LyricUtils lyricUtils = new LyricUtils();
            lyricUtils.readLyricFile(file);
            show_lyric.setLyrics(lyricUtils.getLyrics());////歌词列表
    ...
    private void setViewData() {
        try {
            ...
            setPlayAndPauseStatus();//设置播放和暂停按钮的状态
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
